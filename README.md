...
### 编程范式
PHP是支持很多的编程技术的灵活的、动态语言。它已经发生了巨大的改变，5.0版本引入了面向对象，5.3版本引入了匿名函数和命名空间，5.4版本引入了traits。

#### 面向对象编程

PHP完全支持了面向对象的特性，如类、抽象类、接口、集成、克隆和异常等等。

>* 关于PHP面向对象，[阅读](http://php.net/manual/zh/language.oop5.php)
>* 关于PHPtraits，[阅读](http://php.net/language.oop5.traits)

#### 函数式编程
PHP完美的支持函数，它可以赋值给变量。不管是用户自定义函数，还是内置函数，都可以被引用和动态调用。它们可以作为参数传递给其他方法，也可以作为其他函数的返回值。

PHP也支持方法的递归调用，但是大部分PHP代码关注的是迭代。

PHP5.3开始支持新的匿名函数和闭包。

PHP5.4开始支持在对象的作用域里绑定闭包，并且支持在任何情况下，回调和匿名可以交替使用。

>* 函数式编程，[阅读](http://www.phptherightway.com/pages/Functional-Programming.html)
>* 匿名函数，[阅读](http://php.net/functions.anonymous)
>* 闭包类，[阅读](http://php.net/class.closure)
>* 闭包RFC更多内容，[阅读](https://wiki.php.net/rfc/closures)
>* 回调，[阅读](http://php.net/language.types.callable)　
>* 动态调用函数，[阅读](http://php.net/function.call-user-func-array)


#### 元编程
PHP借助反射API和魔术方法来支持元编程。它有非常多的魔术方法，如：`__get()`, `__set()`, `__clone()`, `__toString()`, `__invoke()`等等，允许开发者在类行为中添加钩子。Ruby程序员嘲笑PHP没有`method_missing`，其实我们也有`__call()`和`__callStatic()`。

>* 魔术方法，[阅读](http://php.net/language.oop5.magic)
>* 反射API，[阅读](http://php.net/intro.reflection)
>* 重载，[阅读](http://php.net/language.oop5.overloading)
